...
func (calc *UpdatePriorityCalculator) AddPod(pod *apiv1.Pod, vpa *vpa_types.VerticalPodAutoscaler, now time.Time) {
	processedRecommendation, _, err := calc.recommendationProcessor.Apply(vpa.Status.Recommendation, calc.resourcesPolicy, calc.conditions, pod)
	if err != nil {
	...
	updatePriority := calc.priorityProcessor.GetUpdatePriority(pod, vpa, processedRecommendation)
