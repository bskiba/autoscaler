...
func TestSortPriority(t *testing.T) {
	...
	calculator := NewUpdatePriorityCalculator(nil, nil, nil, &test.FakeRecommendationProcessor{}, priorityProcessor)

	timestampNow := pod1.Status.StartTime.Time.Add(time.Hour * 24)
	calculator.AddPod(pod1, vpa, timestampNow)
	calculator.AddPod(pod2, vpa, timestampNow)
	calculator.AddPod(pod3, vpa, timestampNow)
	calculator.AddPod(pod4, vpa, timestampNow)
...
func TestUseProcessor(t *testing.T) {
	...
	calculator := NewUpdatePriorityCalculator(
		nil, nil, nil, recommendationProcessor, priorityProcessor)
...
func TestUpdateLonglivedPods(t *testing.T) {
	...
	calculator := NewUpdatePriorityCalculator(
		nil, nil, &UpdateConfig{MinChangePriority: 0.5}, &test.FakeRecommendationProcessor{}, priorityProcessor)

	// Pretend that the test pods started 13 hours ago.
	timestampNow := pods[0].Status.StartTime.Time.Add(time.Hour * 13)
	for i := 0; i < 3; i++ {
		calculator.AddPod(pods[i], vpa, timestampNow)
	}
	result := calculator.GetSortedPods(NewDefaultPodEvictionAdmission())
...
func TestUpdatePodWithQuickOOM(t *testing.T) {
	...
	calculator.AddPod(pod, vpa, timestampNow)
	result := calculator.GetSortedPods(NewDefaultPodEvictionAdmission())
...
func TestDontUpdatePodWithQuickOOMNoResourceChange(t *testing.T) {
	...
	calculator := NewUpdatePriorityCalculator(
		nil, nil, &UpdateConfig{MinChangePriority: 0.1}, &test.FakeRecommendationProcessor{}, priorityProcessor)
	...
	for _, tc := range tests {
		t.Run(fmt.Sprintf("test case: %s", tc.name), func(t *testing.T) {
			...
			priorityProcessor := NewFakeProcessor(map[string]podPriority{
				"POD1": {scaleUp: true, resourceDiff: 0.25}})
			calculator := NewUpdatePriorityCalculator(
				resourcePolicy, nil, &UpdateConfig{MinChangePriority: 0.5}, &test.FakeRecommendationProcessor{}, priorityProcessor)
...
func TestAdmission(t *testing.T) {
	...
	calculator := NewUpdatePriorityCalculator(nil, nil, nil,
